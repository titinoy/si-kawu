...
<body class="font-sans text-slate-800 bg-slate-50 antialiased">
    ...
    <nav class="fixed w-full z-50 top-0 border-b border-gray-200 bg-white/90 backdrop-blur-md shadow-sm">
        <div class="max-w-screen-xl flex items-center justify-between mx-auto px-4 py-3">
            ...
            <a href="{{ route('home') }}" class="text-sm text-slate-600 hover:text-primary font-medium transition-colors flex items-center gap-1">
    ...
    <main class="max-w-screen-xl mx-auto px-4 py-10">
        ...
        <div id="panel-pengumuman">
            ...
            <div class="flex flex-wrap gap-2 mb-6">
                ...
                <a href="{{ route('layanan', $val ? ['kategori' => $val] : []) }}"
                   class="px-4 py-1.5 rounded-full text-sm font-semibold border transition-all
                          {{ request('kategori') == $val ? 'bg-slate-900 text-white border-slate-900' : 'bg-white text-slate-600 border-slate-200 hover:border-slate-400' }}">
                    {{ $label }}
                </a>
                @endforeach
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm card-hover overflow-hidden flex flex-col" id="pengumuman-{{ $p->id }}">
                    <!-- Header Badge -->
                    <div class="px-5 pt-5 pb-3">
                        ...
                        <h3 class="font-bold text-slate-900 text-base leading-snug">{{ $p->judul }}</h3>
                        <p class="text-xs text-slate-400 mt-2">
                            {{ $p->tanggal_tayang->isoFormat('D MMMM Y') }}
                            @if($p->tanggal_berakhir)
                            <span>· s/d {{ $p->tanggal_berakhir->isoFormat('D MMMM Y') }}</span>
                            @endif
                        </p>
                    </div>
                    <!-- Konten Preview -->
                    ...
                    <!-- Expand -->
                    <div class="px-5 pb-5">
                        <button onclick="toggleKonten({{ $p->id }})"
                                class="text-primary text-sm font-semibold hover:underline mt-1 flex items-center gap-1" id="btn-{{ $p->id }}">
                            Baca selengkapnya
                            <svg class="w-3 h-3 transition-transform" id="icon-{{ $p->id }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/></svg>
                        </button>
                        <div id="full-{{ $p->id }}" class="hidden mt-4 prose prose-sm max-w-none text-slate-700">
                            {!! $p->konten !!}
                        </div>
                    </div>
                </div>
                ...
                <form action="{{ route('laporan.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nama_pelapor" value="{{ old('nama_pelapor') }}" required
                                   placeholder="Nama Anda" class="form-input @error('nama_pelapor') border-red-400 @enderror">
                            @error('nama_pelapor')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                        </div>
                        <div>
                            <label class="form-label">No. HP / WhatsApp <span class="text-red-500">*</span></label>
                            <input type="text" name="kontak_pelapor" value="{{ old('kontak_pelapor') }}" required
                                   placeholder="08xxxxxxxxxx" class="form-input @error('kontak_pelapor') border-red-400 @enderror">
                            @error('kontak_pelapor')<p class="mt-1 text-xs text-red-500">{{ $message }}</p>@enderror
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="kategori" class="form-input @error('kategori') border-red-400 @enderror" required>
                                ...
                                <option value="jalan" {{ old('kategori') == 'jalan' ? 'selected' : '' }}>🛣️ Jalan & Transportasi</option>
                                <option value="fasilitas" {{ old('kategori') == 'fasilitas' ? 'selected' : '' }}>🏗️ Fasilitas Umum</option>
                    ...
                    <div>
                        ...
                        <input type="text" name="judul" value="{{ old('judul') }}" required
                               placeholder="Singkat dan jelas" class="form-input @error('judul') border-red-400 @enderror">
